...
For the choice of $\theta = 0$, figure \ref{fig:conv1} represents the variation of the LTE when changing the size of  $h$ and $k$. In this case, we set the value of $k$ to be equal to $\frac{h^2}{2}$ so that the eigenvalues of the system lie within the unit circle. The dashed lines, which represent the theoretical order of the system represent an upper bound to the empirical curves $O(h^2)$ and $O(k)$. It is easy to see that in both comparisons the slope of the LTE in logarithmic scale is greater than the slope of the help lines.
