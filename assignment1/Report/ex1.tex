...
Figure \ref{fig:global1} represents the global error at time $t = 10$ made by the one step  and multistep methods for different step sizes. As expected, the size the global error decreases when increasing the number of points used in the approximation.
...
More sophisticated algorithms, such as DOPRI54, use embbeded methods of lower order to estimate the error. Even though the estimations obtained are not as good as with step doubling, the secondary method is closely related to the main algorithm so that they can share computations and thus be very efficient. 
